...
class RentingHistoryScreen extends StatefulWidget {
  ...
  @override
  State<RentingHistoryScreen> createState() =>
      _RentingHistoryScreenState();
}
...
class _RentingHistoryScreenState extends State<RentingHistoryScreen> {
  ...
  void _loadHistory() {
    ...
    if (widget.deviceId == null) {
      final allRents = provider.devices
          .expand((d) => d.rentingHistory)
          .toList();

      allRents.sort(
            (a, b) => DateTime.parse(b.rentStart)
            .compareTo(DateTime.parse(a.rentStart)),
      );

      _allHistory = allRents;
    } else {
      final device = provider.devices
          .firstWhere((d) => d.id == widget.deviceId);
  ...
  void _updateCurrentDevice() {
    ...
    _currentDevice = provider.devices.firstWhere(
          (d) => d.id == _history[_currentIndex].deviceId,
    );
  }
    ...
    setState(() {
      if (value == null || value.isEmpty) {
        ...
        _history = _allHistory.where((rent) {
          final addressMatch =
              rent.renterLocation?.address
                  .toLowerCase()
                  .contains(query) ??
                  false;

          final renterMatch =
          rent.renterName.toLowerCase().contains(query);
  ...
  void _deleteCurrentRent() {
    ...
    final device =
    provider.devices.firstWhere((d) => d.id == deviceId);

    device.rentingHistory
        .removeWhere((rent) => rent.id == rentId);
